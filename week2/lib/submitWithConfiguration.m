...
function response = getResponse(url, body)
  ...
  if ispc
    ...
    json_command = sprintf('curl -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -s -d "%s" --ssl-no-revoke "%s"', new_body, url);
    ...
    json_command = sprintf('curl -X POST -H "Cache-Control: no-cache" -H "Content-Type: application/json" -s -d '' %s '' --ssl-no-revoke ''%s''', body, url);
